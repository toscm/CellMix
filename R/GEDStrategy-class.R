...
# generator function for in/out data check function
.in_out_GFUN <- function(prefix){
    function(x){
    
        if( missing(x) ) x <- gedAlgorithm()
        f <- sys.function()
        if( is.character(x) && length(x) > 1L ) return(t(sapply(x, f)))
        
        prefix_pattern <- paste0("^", prefix)
        req <- 
                if( is(x, 'GEDalgorithm_entry') ){
                    req <- grep(prefix_pattern, names(x), value=TRUE)
                    unlist(x[req])
                }else{
                    # load the algorithm
                    x <- GEDStrategy(x)
                    req <- grep(prefix_pattern, slotNames(x), value=TRUE)
                    vapply(req, function(s) slot(x, s), logical(1))		
                }
        
        setNames(req, sub(prefix_pattern,"",names(req)))
    }
}

#' Checking Input and Output Data for GED Algorithms
#'  
#' \code{gedInput} tells which input data is required by a given, 
#' or all GED algorithm(s).
#' 
#' If \code{x} is not missing \code{gedInput} returns a logical vector with one 
...
#' gedInput('qprog')
#' 
gedInput <- .in_out_GFUN('req')
gedRequired <- gedInput
#' Similarly \code{gedOutput} tells the type of output returned by each 
#' \code{ged} algorithm.
#' 
#' @export
#' @rdname GEDalgorithm-utils
...
#' gedOutput('csSAM')
#' 
gedOutput <- .in_out_GFUN('out')

#' \code{gedIO} combines the information about input data and ouput data in 
#' a compact way. 
#' 
#' @export
#' @rdname GEDalgorithm-utils
...
gedIO <- function(x = gedAlgorithm()){
    ...
    if( !is.matrix(input) ){
        cn <- names(input)
        input <- matrix(input, 1L, byrow = TRUE)
        output <- matrix(output, 1L, byrow = TRUE)
        colnames(input) <- colnames(output) <- cn
    }
    # sanity asserts
    stopifnot( all.equal(dim(input), dim(output)) )
    stopifnot( all.equal(dimnames(input), dimnames(output)) )
    
    d <- dim(input)
    dn <- dimnames(input)
    input <- ifelse(input, 'I', '')
    output <- ifelse(output, 'O', '')
    res <- mapply(function(i, o){
                s <- if( nzchar(i) ) i else ''
                if( nzchar(o) ) s <- paste0(s, if(nzchar(s)) '/', o)
                if( nzchar(s) ) s else '-'
            }
           , as.vector(input), as.vector(output))
    
   res <- matrix(res, d[1L], d[2L], dimnames = dn)
   
   if( length(x) == 1L || is(x, 'GEDalgorithm_entry')) setNames(res[1L, ], colnames(res))
   else res
}
